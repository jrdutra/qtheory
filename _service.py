...
def _eval_service_minutes_occurrence(start_service=[], end_service=[]):
    ...
    start_service = _utils._cls_not_str(start_service)
    end_service = _utils._cls_not_str(end_service)

    #Execute if both pair is OK by data format or same size.
    if _utils._verify_vect_pair(start_service, end_service):
        ...
        df_repetitions = _utils._build_df(list_time, list_repetitions, 'minutes', 'repetitions')
        return df_repetitions

def _eval_approximate_duration(start_service=[], end_service=[]):
    df = _utils._build_df(start_service, end_service, 'start_service', 'end_service')
